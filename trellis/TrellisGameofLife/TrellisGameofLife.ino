...
#define NUMBER_OF_SEQUENT_SUCCESSES 100
...
void writeSequence(int *sequence, int len){
  for(int i=0; i < len; i++){
    trellis.setLED(sequence[i]);
  }
  trellis.writeDisplay();
}

void revealSecret(){
  Serial.println("Reveal Secret");
  for(int i=0; i < 3; i++){
    resetBoard();
    delay(100);
    writeSequence(one, oneLen);
    delay(500);
  }
  resetBoard();
}
...
void loop() {
  ...
  if(successCount > NUMBER_OF_SEQUENT_SUCCESSES){
    revealSecret();
    successCount = 0;
    resetBoard();
  }
